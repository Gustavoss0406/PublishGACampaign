...
def upload_image_asset(client: GoogleAdsClient, customer_id: str, image_url: str, process: bool = False) -> str:
    logging.info(f"Download da mídia: {image_url}")
    ...
    resource = result.results[0].resource_name
    logging.info(f"Imagem enviada: {resource}")
    return resource
...
def upload_square_image_asset(client: GoogleAdsClient, customer_id: str, image_url: str) -> str:
    ...
    result = svc.mutate_assets(customer_id=customer_id, operations=[op])
    resource = result.results[0].resource_name
    logging.info(f"Imagem quadrada enviada: {resource}")
    return resource
...
def get_customer_id(client: GoogleAdsClient) -> str:
    ...
    if not accessible.resource_names:
        raise Exception("Nenhum customer acessível encontrado.")
    return accessible.resource_names[0].split("/")[-1]
    ...
    def clean_cover_photo(cls, value):
        if isinstance(value, str):
            ...
            if v and not urlparse(v).scheme:
                v = "http://" + v
            logging.debug(f"Cover photo limpa: {v}")
            return v
        return value
